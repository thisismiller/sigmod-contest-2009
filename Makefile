...
# our test cases
lib.so:	$(SRCS) $(HDRS)
	$(CC) $(CFLAGS) -fPIC -shared $(BDBINC) $(BDBLIBS) ./bdbimpl.c -o lib.so
	
.c.so:
	$(CC) $(CFLAGS) -fPIC -shared $(BDBINC) $(BDBLIBS) $*.c -o $*.so

$(PROG): $(SRCS) $(HDRS) lib.so
	$(CC) $(CFLAGS) $(SRCS) ./lib.so -pthread -o $(PROG)
...
lib.dylib: $(SRCS) $(HDRS)
	$(CC) $(CFLAGS) -fPIC -dynamiclib $(BDBINC) $(BDBLIBSMACOS) bdbimpl.c -o lib.dylib

.c.dylib:
	$(CC) $(CFLAGS) -dynamiclib $(BDBINC) $(BDBLIBSMACOS) $*.c -o $*.dylib

dummy:
	$(CC) $(CFLAGS) -fPIC -shared ./dummyimpl.c -o lib.so

dummymacos:
	$(CC) $(CFLAGS) -fPIC -dynamiclib ./dummyimpl.c -o lib.dylib

macos:  $(SRCS) $(HDRS) lib.dylib
	$(CC) $(CFLAGS) $(SRCS) lib.dylib -o $(PROG)
